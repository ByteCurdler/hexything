...
import math
from modifyclass import add_method
...
class Hex:
    ...
    def __init__(self, x, y=None, z=None):
        print(type(x))
        if type(x) is list or type(x) is Cube:
            z = x[2]
            y = x[1]
            x = x[0]

        assert not (x + y + z != 0), "x + y + z must be 0"
        self.hex = Cube(x=x, y=y, z=z)
        self.data = Hex.hex_grid.get_data(self.key())
    
    def key(self):
        return "{:+}{:+}{:+}".format(self.hex.x, self.hex.y, self.hex.z)
    
    def coordinates(self):
        return [self.hex.x, self.hex.y, self.hex.z]
        
    def add(self, hex):
        return Hex(self.hex.x + hex.x, self.hex.y + hex.y, self.hex.z + hex.z)
    # 
    # def subtract(self, b):
    #     return Hex(self.hex.x - b.hex.x, self.hex.y - b.hex.y, self.hex.z - b.hex.z)
    # 
    # def scale(a, scale):
    #     return Hex(self.hex.x * scale, self.hex.y * scale, self.hex.z * scale)
...
# 
    def neighbor(self, direction):
        return self.add(Hex.__hex_directions[direction])

    def diagonal(self, direction):
        return self.add(Hex.__hex_diagonals[direction])

    def __magnitude(self):
        return abs(self.x) + abs(self.y) + abs(self.z)

    def distance(self, b):
        return self.subtract(b).__magnitude() // 2
